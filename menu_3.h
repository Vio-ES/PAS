...
void menu_3(long int *total_pengeluaran){
  ...
  do{
  	...
  	printf("\nTambah list? 1. YA 2. TIDAK\n");
		scanf("%d", &yesno);
	} while (yesno==1); //linked list terus bertambah selama user menginginkannya
...
void catatan_pengeluaran(Pengeluaran **first){
		...
		//membuat temporary struct untuk menerima input user
    Pengeluaran *pengeluaran_baru = (Pengeluaran*)malloc(sizeof(Pengeluaran));
		
    //menerima input user
    printf("Nama pengeluaran    : ");
		scanf("%s", pengeluaran_baru->nama_pengeluaran);
		printf("Nominal pengeluaran : Rp");
		scanf("%ld", &(pengeluaran_baru->Nominal));
	
   	pengeluaran_baru->link = NULL;

	    if(*first == NULL)
	        *first = pengeluaran_baru; //menunjuk alamat struct yang telah dibuat
	    else
	    {
    	    Pengeluaran *lastNode = *first;
    	    while(lastNode->link != NULL)
    	    {
            	lastNode = lastNode->link;
        	}
        	lastNode->link = pengeluaran_baru;
    	}
}
